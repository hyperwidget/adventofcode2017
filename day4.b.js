const test1 = `abcde fghij`
const test1Result = true
const test2 = `abcde xyz ecdab`
const test2Result = false
const test3 = `a ab abc abd abf abj`
const test3Result = true
const test4 = `iiii oiii ooii oooi oooo`
const test4Result = true
const test5 = `oiii ioii iioi iiio`
const test5Result = false
...
const day4 = val => {
  const splitRows = val.split('\n')
  let trueCount = 0

  splitRows.map(row => {
    const splitVals = row.split(' ')
    let sortedSplits = splitVals.map(val =>
      val
        .split('')
        .sort()
        .join('')
    ...
    if (sortedSplits.length == new Set(sortedSplits).size) {
      trueCount++
    }
  })
