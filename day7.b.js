...
const day7 = val => {
  ...
  splitValues.map(val => {
    ...
    let rootVals = splitVal[0].split(' ')
    tree[rootVals[0]] = {
      weight: parseInt(rootVals[1].replace('(', '').replace(')', ''))
    }
  })

  // Add children and their weights
  const filteredVals = splitValues.filter(val => val.indexOf('->') !== -1)
  filteredVals.map(val => {
    let splitVal = val.split('-> ')
    let children = splitVal[1].split(', ')
    if (splitVal[0].split(' ')[0] in tree) {
      tree[splitVal[0].split(' ')[0]].children = children.map(child => {
        let temp = tree[child]
  ...
  // Prune it all down to a single node with child nodes
  nodesToRemove.map(node => delete tree[node])
  const rootNode = tree[Object.keys(tree)[0]]

  // Recursively get the sums of children weights
  recursionSum(rootNode, 0)

  let solution = recursionFind(rootNode, { found: false }, rootNode.weight)

  return solution.val
}

const recursionFind = (node, found, targetVal) => {
  if (node.children) {
    const totals = node.children.map(child => child.totalWeight)
    const firstVal = totals[0]
    // At end
    if (totals.every(val => firstVal === val)) {
      found.val = targetVal - node.childWeights
    } else {
      let counts = {}
      node.children.forEach(
        child =>
          (counts[child.totalWeight] = (counts[child.totalWeight] || 0) + 1)
      )
      let targetVal = Object.keys(counts).filter(
        countIndex => counts[countIndex] !== 1
      )[0]

      let badNode = node.children.filter(
        child => child.totalWeight !== targetVal
      )[0]

      recursionFind(badNode, found, targetVal)
    }
  }
...
const recursionSum = (node, sum) => {
  let childWeights = 0
  if (node.children)
    node.children.map(child => {
      childWeights += child.weight
      // console.log(child)
      childWeights += recursionSum(child, 0)
    })

  node.childWeights = childWeights
  node.totalWeight = childWeights + node.weight
  return sum + childWeights
}

const retVal = day7(input)
console.log(retVal)
