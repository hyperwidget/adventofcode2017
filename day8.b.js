const test1 = `b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`
const test1Result = 1

const input = `js inc 257 if wn < 9
jq dec -586 if esb != -3
gcf inc -603 if i >= -9
gcf dec -300 if d != 1
g inc -973 if gy > -1
...
const performInstruction = (registers, instruction) => {
  ...
  if (tempReg[targetReg] === undefined) {
    tempReg[targetReg] = 0
  }
  if (tempReg[condReg] === undefined) {
    tempReg[condReg] = 0
  }

  if (eval(`tempReg[condReg] ${operation} ${condOperation}`)) {
    let changeVal = direction === 'inc' ? +amount : -amount
